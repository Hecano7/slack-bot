...
function calculateDashboardStandupsData(standups, students) {
  ...
  const todaysStandupPercent = Math.round(
    (uniqueStandupsToday.length / students.length) * 100
  );
  return {
    summary: [
      {
...
function calculateDashboardCheckinData(activeCheckins, students) {
  ...
  const checkinPercent = Math.round(
    (activeCheckins.length / students.length) * 100
  );
...
function calculateAbsentees(activeCheckins, students) {
  ...
  var sperateStudentsCheckins = [];
  for (var i = 0; i < students.length; i++) {
    let individualStudents = activeCheckins.filter(individual => individual.slack_id == students[i].slack_id && (individual.notAtSchool == false));
    sperateStudentsCheckins.push(individualStudents);
  }
  var slackId = [];
  for (var i = 0; i < sperateStudentsCheckins.length; i++) {
    sperateStudentsCheckins[i].map(date => {
      var extract = date.checkin_time.slice(0, 10);
      if (extract == today) {
        slackId.push(date.slack_id);
      }
    });
  }
  var absentees = [];
  for (var i = 0; i < students.length; i++) {
    let absent = students.filter(individual => individual.slack_id == students[i].slack_id);
    if (!slackId.includes(students[i].slack_id)){
      absentees.push(absent[0]);
    }
  }
  return absentees;
}
...
function calculateIndividualStandupsData(standups) {
  ...
  const standupsLastSevenDaysDates = standupsLastSevenDays.map(standup => {
    ...
    return `${standupDate.getFullYear()} - ${standupDate.getMonth() +
      1} - ${standupDate.getDate()}`;
  });
  const uniqueStandupsLastSevenDays = [...new Set(standupsLastSevenDaysDates)];

  const weekOfStandupsPercent = Math.round(
    (uniqueStandupsLastSevenDays.length / 7) * 100
  );
  ...
  const totalDaysEnrolled = Math.round(
    (tomorrowMidnight - dayOneMidnight) / millisecondsToDays
  );
  ...
  const averageStandupPercent = Math.round(
    (uniqueDaysWithStandups.length / totalDaysEnrolled) * 100
  );
  return [
    {
...
function calculateIndividualCheckinData(checkins) {
  ...
  checkins.forEach(checkin => {
    if (!checkin.checkout_time) {
      checkin.hours =
        (new Date() - new Date(checkin.checkin_time)) / millisecondsToHours;
    }
  });
  ...
  let timeSpentLastSevenDays = checkinsLastSevenDays.reduce(
    (accumulator, checkin) => {
      return accumulator + checkin.hours;
    },
    0
  ...
  let autoCheckoutsLastSevenDays = completedCheckinsLastSevenDays.reduce(
    (accumulator, checkin) => {
      if (checkin.auto_checkout) {
        accumulator++;
      }
      return accumulator;
    },
...
function calculateIndividualWakatimeData(wt) {
  ...
  let timeSpentLastSevenDays = wakatimesLastSevenDays.reduce(
    (accumulator, wakatime) => {
      return accumulator + parseInt(wakatime.duration);
    },
    0
  );
